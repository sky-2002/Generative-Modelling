...
class Block(nn.Module):
    def __init__(
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReLU(0.2) if act == "relu" else nn.LeakyReLU(0.2),
        )
...
class Generator(nn.Module):
    def __init__(self, in_channels=3, features=64):
        ...
        self.down1 = Block(features, features * 2, act="leaky", use_dropout=False)  # 64
        self.down2 = Block(
            features * 2, features * 4, act="leaky", use_dropout=False
        )  # 32
        self.down3 = Block(
            features * 4, features * 8, act="leaky", use_dropout=False
        )  # 16
        self.down4 = Block(
            features * 8, features * 8, act="leaky", use_dropout=False
        )  # 8
        self.down5 = Block(
            features * 8, features * 8, act="leaky", use_dropout=False
        )  # 4
